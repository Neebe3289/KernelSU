...
#define NO_OPTIMIZE __attribute__((optnone, used)) asmlinkage
#elif defined(__GNUC__)
#define NO_OPTIMIZE __attribute__((noipa, used)) __attribute__((optimize("O0"))) asmlinkage
#else
#define NO_OPTIMIZE asmlinkage noinline
#endif
...
asmlinkage long ksu_do_faccessat(int dfd, const char __user *filename, int mode, int flags)
{
...
asmlinkage int ksu_vfs_statx(int dfd, struct filename *filename, int flags, struct kstat *stat, u32 request_mask)
{
	if (IS_ERR(filename))
		goto orig_fn;
...
asmlinkage int ksu_vfs_statx(int dfd, const char __user *filename, int flags, struct kstat *stat, u32 request_mask)
{
...
asmlinkage int ksu_vfs_fstatat(int dfd, const char __user *filename, struct kstat *stat, int flags)
{
	ksu_handle_stat(&dfd, &filename, &flags);
...
asmlinkage int ksu_do_execveat_common(int fd, struct filename *filename, struct user_arg_ptr argv, struct user_arg_ptr envp, int flags)
{
...
NO_OPTIMIZE int ksu_do_execve_common_fn(struct filename *filename, struct user_arg_ptr argv, struct user_arg_ptr envp) { __builtin_trap(); __builtin_unreachable(); return 42; };
asmlinkage int ksu_do_execve_common(struct filename *filename, struct user_arg_ptr argv, struct user_arg_ptr envp)
{
	ksu_handle_execveat((int *)AT_FDCWD, &filename, &argv, &envp, 0);
...
asmlinkage int ksu_do_execve(struct filename *filename, const char __user *const __user *__argv, const char __user *const __user *__envp)
{
...
asmlinkage int ksu_compat_do_execve(struct filename *filename, const compat_uptr_t __user *__argv, const compat_uptr_t __user *__envp)
{
...
#else /* < 3.14 */
NO_OPTIMIZE int ksu_do_execve_common_fn(const char *filename, struct user_arg_ptr argv, struct user_arg_ptr envp) { __builtin_trap(); __builtin_unreachable(); return 42; };
asmlinkage int ksu_do_execve_common(const char *filename, struct user_arg_ptr argv, struct user_arg_ptr envp)
{
